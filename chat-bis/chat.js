	...
	$
			.ajax({
				...
				success : function(msg) {
					...
					$("#statusResponse")
							.html(
									'<span style="color:green">Le statut a &eacute;t&eacute; mis &agrave; jour</span>');
					setTimeout(rmResponse, 3000);
				},
				error : function(msg) {
					// On affiche l'erreur dans la zone de réponse
					$("#statusResponse").html(
							'<span style="color:orange">Erreur</span>');
					setTimeout(rmResponse, 3000);
				}
			});
}
...
function getOnlineUsers() {
	...
	$
			.getJSON(
					'phpscripts/get-online.php',
					function(data) {
						// Si data['error'] renvoi 0, alors ça veut dire que
						...
						if (data['error'] == '0') {
							...
							for ( var id in data['list']) {
								...
								// On met dans la variable text le statut en
								// toute lettre
								...
								// On affiche d'abord le lien pour insérer le
								// pseudo dans la zone de texte
								online += '<a href="#post" onclick="insertLogin(\''
										+ data['list'][id]["login"]
										+ '\')" title="' + text + '">';
								// Ensuite on affiche l'image
								online += '<img src="status-' + image
										+ '.png" /> ';
								// Enfin on affiche le pseudo
								online += data['list'][id]["login"] + '</a>';

								// Si i vaut 1, ça veut dire qu'on a affiché un
								// membre
							...
							$("#users")
									.html(
											'<span style="color:gray;">Aucun utilisateur connect&eacute;.</span>');
					});
}
...
function getMessages() {
	...
	// On lance la requête ajax
	$.getJSON('phpscripts/get-message.php?dateConnexion='
			+ $("#dateConnexion").val(), function(data) {
		/*
		 * On vérifie que error vaut 0, ce qui signifie qu'il n'y aucune erreur
		 */
		if (data['error'] == '0') {
			...
			$("#annonce").html(
					'<span class="info"><b>' + data['annonce']
							+ '</b></span><br /><br />');
			...
			/*
			 * Si error vaut unlog, alors l'utilisateur connecté n'a pas de
			 * compte. Il faut le rediriger vers la page de connexion
			 */
			$("#annonce").html('');
			$("#text").html('');
...
function postMessage() {
	...
	$.ajax({
		...
		success : function(msg) {
			...
			// on resélectionne la zone de texte, en cas d'utilisation du bouton
			// "Envoyer"
			$("#message").focus();
		},
		error : function(msg) {
